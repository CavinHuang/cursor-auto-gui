...
def build_macos_arm64():
    ...
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'dmgbuild'])  # 用于创建DMG包
...
<dict>
        ...
        if os.path.exists(macos_dir):
            for file in os.listdir(macos_dir):
                ...
                if os.path.isfile(exe_path):
                    print(f"  - {file}" + (" (可执行)" if os.access(exe_path, os.X_OK) else ""))
                    if os.path.isfile(exe_path) and not os.access(exe_path, os.X_OK):
    ...
    print('安装包已准备就绪:')
    print('1. PKG格式: dist/CursorPro_Installer_ARM64.pkg')
    print('2. DMG格式: dist/CursorPro_ARM64.dmg')
...
def create_macos_dmg():
    ...
    try:
        # 检查dmgbuild是否已安装
        subprocess.check_call([sys.executable, '-m', 'dmgbuild', '--help'],
                              stdout=subprocess.DEVNULL,
                              stderr=subprocess.DEVNULL)
